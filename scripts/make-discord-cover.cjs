...
/* Renders scripts/discord-cover.html to a 1024x576 PNG for Discord's
   "Rich Presence Invite Image" (Developer Portal -> Rich Presence -> Art Assets
   -> Cover Image), which is the banner on the "Invite to join" card in chat.

   Run:  npx electron scripts/make-discord-cover.js
   Out:  screenshots/discord-cover.png                                       */
...
const W = 1024;
const H = 576; // Discord requires 16:9 at 1024x576.
const SRC = path.join(__dirname, 'discord-cover.html');
const OUT = path.join(__dirname, '..', 'screenshots', 'discord-cover.png');
