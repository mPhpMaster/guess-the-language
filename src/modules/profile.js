...
export function openPlayerCard(player) {
    ...
    /* Promote another (non-host, non-spectator) player to host.
       ...
       A site admin who joined as a plain player can't prove admin-ness to that RPC at
       all, so they go through /api/admin, where the signed `adm` claim is actually
       verified and the write runs with the service-role key; they may also promote
       mid-round. Widening the gate alone would be useless — the room RPC would still
       reject them — and dropping its check would let anyone steal host. */
    const mkHost = $('#btn-player-card-makehost');
    if (mkHost) {
        const promotable = !isYou && !player.is_host && !player.spectator && !!room;
        const asHost = mp.isAdmin && promotable && room?.status === 'lobby';
        const asAdmin = !mp.isAdmin && isAdmin() && promotable;
        mkHost.classList.toggle('hidden', !(asHost || asAdmin));
        mkHost.textContent = `👑 ${t('makeHost')}`;
        mkHost.onclick = async () => {
            try {
                if (asAdmin) await adminApi('make_host', { roomId: room.id, targetPlayerId: player.id });
                else await window.GTL_MULTIPLAYER.makeHost(player.id);
                closePlayerCard();
