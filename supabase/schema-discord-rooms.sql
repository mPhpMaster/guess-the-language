...
-- v3.10 override: auto-assign host on join so a hostless Discord room (host left /
-- was cleaned up) promotes the earliest remaining player instead of leaving
-- everyone on "Waiting for host…". Mirrors migration join_discord_room_ensure_host.
create or replace function public.join_discord_room(
...
grant execute on function public.join_discord_room(text, text, jsonb, text, text) to anon, authenticated;
